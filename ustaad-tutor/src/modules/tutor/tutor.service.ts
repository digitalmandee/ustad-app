      ...
      if (sessionInfo) {
        ...
        try {
          ...
          if (parentUser && parentUser.email) {
            const { sendEmailViaSES } = await import(
              "../../services/aws-email.service"
            );
            if (
              data.status === TutorSessionStatus.TUTOR_HOLIDAY ||
              data.status === TutorSessionStatus.CANCELLED_BY_TUTOR
            ) {
              const subject =
                data.status === TutorSessionStatus.TUTOR_HOLIDAY
              ...
              console.log(
                `📧 Session status email sent to parent: ${parentUser.email}`
              );
            }
          }
        } catch (emailErr) {
          ...
          try {
            ...
            if (parentUser && parentUser.email) {
              ...
              if (
                ...
                await sendEmailViaSES(parentUser.email, subject, html);
                console.log(
                  `📧 Session edit email sent to parent: ${parentUser.email}`
                );
              }
            }
          } catch (emailErr) {
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          console.log(
            `📧 Contract dispute email sent to parent: ${parent.email}`
          );
        } else if (
          status === ParentSubscriptionStatus.PENDING_COMPLETION &&
          parent &&
          ...
          await sendEmailViaSES(parent.email, subject, html);
          console.log(
            `📧 Contract completion email sent to parent: ${parent.email}`
          );
        }
      } catch (emailErr) {
