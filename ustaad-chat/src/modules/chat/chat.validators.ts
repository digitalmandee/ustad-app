...
export const createMessageValidator = () => {
  return [
    ...
    body('content')
      .if(body('type').equals(MessageType.TEXT))
      .exists()
    ...
    body('replyToId').optional().isUUID().withMessage('replyToId must be a valid UUID'),
    ...
    body('offer')
      ...
      .exists()
      .withMessage('offer is required when type is OFFER')
      .bail()
      .isObject()
    ...
    body('offer.receiverId')
      ...
      .exists()
      .withMessage('offer.receiverId is required')
      .bail()
      .isUUID()
      .withMessage('offer.receiverId must be a valid UUID'),

    body('offer.childName')
      .if(body('type').equals('OFFER'))
      .exists()
      .withMessage('offer.childName is required')
      .bail()
      .isString()
      .withMessage('offer.childName must be a string')
      .notEmpty()
      .withMessage('offer.childName cannot be empty'),

    body('offer.amountMonthly')
      .if(body('type').equals('OFFER'))
      .exists()
      .withMessage('offer.amountMonthly is required')
      .bail()
      .isDecimal({ decimal_digits: '0,2' })
      .withMessage('offer.amountMonthly must be a decimal number with up to 2 decimal places'),
    ...
    body('offer.startDate')
      .if(body('type').equals('OFFER'))
      .exists()
      .withMessage('offer.startDate is required')
      .bail()
      .isISO8601()
      .toDate()
      .withMessage('offer.startDate must be a valid date'),

    body('offer.startTime')
      .if(body('type').equals('OFFER'))
      .exists()
      .withMessage('offer.startTime is required')
      .bail()
      .matches(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/)
      .withMessage('offer.startTime must be a valid time in HH:MM format'),

    body('offer.endTime')
      .if(body('type').equals('OFFER'))
      .exists()
      .withMessage('offer.endTime is required')
      .bail()
      .matches(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/)
      .withMessage('offer.endTime must be a valid time in HH:MM format')
      .custom((endTime, { req }) => {
    ...
    body('fileId')
      .if(body('type').isIn([MessageType.FILE, MessageType.IMAGE, MessageType.AUDIO]))
      .exists()
      .withMessage('fileId is required for file/image/audio messages')
      .bail()
      .isUUID()
